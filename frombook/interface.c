...
GtkWidget *create_winMain (InterFace *ui)
{
   ...
   //定义顶层工具栏
   GtkWidget *vbHead;
   vbHead = gtk_hbox_new (FALSE, 0);
   gtk_widget_show (vbHead);
   gtk_box_pack_start (GTK_BOX (vbMain), vbHead, TRUE, TRUE, 0);
   ...
   gtk_box_pack_start (GTK_BOX (vbMain), hbState
    */
   ...
   gtk_widget_set_can_default (btPre,TRUE);
   GtkWidget *image1;
   ...
   gtk_widget_set_can_default (btPlay,TRUE);
   GtkWidget *image2;
   ...
   gtk_widget_set_can_default (btPause,TRUE);
   GtkWidget *image3;
   ...
   gtk_widget_set_can_default (btStop,TRUE);
   GtkWidget *image4;
...
GtkWidget *create_login_window_with_phone(InterFace *ui) //使用手机号进行登录的页
{
  ...
  //  gtk_widget_show(window_zc_1);
     ...
     gtk_container_add (GTK_CONTAINER (window_login_with_phone), vb_Main_zc_1);
         ...
         gtk_entry_set_text(GTK_ENTRY(entry), "请输入手机号");
         // 修改文本输入构件中的文本
         ...
         gtk_entry_set_max_length(GTK_ENTRY (entry_1), 50);
              ...
              vb_Main_zc_1_1 = gtk_hbox_new (FALSE, 0);
              gtk_widget_show (vb_Main_zc_1_1);
              gtk_box_pack_start(GTK_BOX (vb_Main_zc_1), vb_Main_zc_1_1,FALSE,FALSE,0);
...
GtkWidget *create_diaMedialib (InterFace *ui)
{
   ...
   dialog_vbox4=G_TYPE_CHECK_INSTANCE_CAST((diaMedialib), GTK_TYPE_DIALOG, GtkDialog);
   ...
   //TODO   g_signal_connect(G_OBJECT(tlbAdddir), "clicked",G_CALLBACK(on_tlbAdddir_clicked), ui);
...
GtkWidget *create_diaVolume (InterFace *ui)
{
   ...
   gtk_widget_set_can_default (vscale1,TRUE);
